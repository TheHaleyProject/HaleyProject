...
{
    public sealed class ImageAPI //Sealing, so that others cannot inherit it as an abstract and develop on their own.
    {
        ...
        public static ImageSource resizeImage(ImageSource input_image, int pixel_height, int pixel_width, resize_affect_mode resize_mode = resize_affect_mode.pixelSize, double dpi = 0)
        {
            try
            {
                ...
                if (base_source == null) return null;
        ...
        public static Image base64ToImage(string input_string)
        {
            try
            {
                if (!isBase64(input_string)) return null; //If not a base 64 string, return null.
        ...
        public static ImageSource bitmapToImageSource(Bitmap input_bitmap)
        {
            try
            {
              var bitmap_data =  input_bitmap.LockBits(new Rectangle(0, 0, input_bitmap.Width, input_bitmap.Height), ImageLockMode.ReadOnly, input_bitmap.PixelFormat);
                BitmapSource _result = BitmapSource.Create(bitmap_data.Width, bitmap_data.Height, input_bitmap.HorizontalResolution, input_bitmap.VerticalResolution, PixelFormats.Bgr32, null,bitmap_data.Scan0, bitmap_data.Stride * bitmap_data.Height, bitmap_data.Stride);
        ...
        public static ImageSource pixelsToImageSource(int _pixel_width, int _pixel_height, byte[] _pixels, System.Windows.Media.PixelFormat _format,double dpiX =96, double dpiY =96,BitmapPalette referenced_palette = null)
        {
            try
            {
                //Calculate stride
                ...
                BitmapSource bmap_source = BitmapSource.Create(_pixel_width, _pixel_height, dpiX, dpiY, _format, referenced_palette, _pixels,_stride); //Sometimes the byte index values may refer some palette. Based on the palette, the color will differ. So if you know that pass that as well, else it will be null and default palette is used.
                bmap_source.Freeze();
                return bmap_source;
            }
            catch (Exception ex)
        ...
        public static bool saveImageSource(ImageSource input_image, string file_path, BitmapEncoder encoder = null)
        {
            try
            {
                ...
                if (base_source != null)
                {
                    ...
                    {
                        if (encoder == null) encoder = new PngBitmapEncoder(); //Whatever the input file format is, we are trying to decode it using png encoder. Check if it will work.
                        encoder.Frames.Add(BitmapFrame.Create(base_source));
        ...
        public static bool isBase64(string input_string)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(input_string) || input_string.Length % 4 != 0 || input_string.Contains(" ") || input_string.Contains("\t") || input_string.Contains("\r") || input_string.Contains("\n")) return false;
        ...
        private static ImageSource _resizeImage_pixelImage(BitmapSource base_source, int pixel_height, int pixel_width, bool fit_width, double dpi, bool maintain_ratio = true)
        {
            ...
            try
            {
                if (dpi == 0) dpi = base_source.DpiX; //Dpi cannot be zero.
                ...
                //SET DPI, PIXELSIZE
                RenderTargetBitmap render_bitmap = new RenderTargetBitmap((int)Math.Round(adjusted_width), (int)Math.Round(adjusted_height), 0, 0, PixelFormats.Default);
        ...
        private static ImageSource _resizeImage_pixelDpi(BitmapSource base_source, int pixel_height, int pixel_width, bool fit_width, double dpi, bool maintain_ratio = true)
        {
            try
            {
                //CHANGE PIXEL SIZE FIRST AND COPY THE DATA EXCEPT DPI
                BitmapSource pixel_modified_source = _resizeImage_pixelOnly(base_source as BitmapSource, pixel_height, pixel_width, fit_width) as BitmapSource;

                //COPYING ALL VALUES, EXCEPT THE DPI.
                if (dpi == 0) dpi = base_source.DpiX;
                ...
                ImageSource result = BitmapSource.Create(pixel_modified_source.PixelWidth, pixel_modified_source.PixelHeight, dpi, dpi, pixel_modified_source.Format, null, empty_pixel_array, _stride);
                return result;
            }
            catch (Exception ex)
            {
        ...
        private static bool _should_fit_with(double source_image_height, double source_image_width)
        {
            try
            {
                //Check which is long. Longer side should fit.
                if (source_image_height > source_image_width) return false; //Height is greater, height should fit inside
